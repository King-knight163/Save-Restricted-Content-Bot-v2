...
async def batch_link(_, message):
    ...
    adjust_value = 0
    if adjust.text == "/addnumber":
        num_msg = await app.ask(message.chat.id, "By how much do you want to increase the numbers?")
        try:
            adjust_value = int(num_msg.text.strip())
        except ValueError:
            await app.send_message(message.chat.id, "Invalid number. Defaulting to 0")
    elif adjust.text == "/lessnumber":
        num_msg = await app.ask(message.chat.id, "By how much do you want to decrease the numbers?")
        try:
            adjust_value = -int(num_msg.text.strip())
        except ValueError:
            await app.send_message(message.chat.id, "Invalid number. Defaulting to 0")
    ...
    pin_msg = await app.send_message(
        user_id,
        f"Batch process started ⚡\nProcessing: 0/{cl}\nNumber adjustment: {'+' if adjust_value > 0 else ''}{adjust_value}\n\n**Powered by Team SPY**",
